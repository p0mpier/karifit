...
try{
    ...
    //トレーニングのセット
    $sql = 'UPDATE account_state SET last_name=?,first_name=?,last_kana=?,first_kana=?,sex=?,birthday=?,address=?,bring=?,profession=?,workplace=?,branch=?,department=?,updated=?,current_course=? WHERE member_code=?';
    $stmt = $dbh->prepare($sql);
    $data = array($last_name,$first_name,$last_kana,$first_kana,$sex,$birthday,$address,$bring,$profession,$workplace,$branch,$department,$updated,$current_course,$member_code);
    $stmt->execute($data);
